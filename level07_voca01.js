...
function handlerAft() {
  location.replace('level07_voca02.html');
}

vocaCard.use().transform('t30, 100').appendTo(topBox);

hanjaArea.text(180, 80, '1 / 43').attr({
  'font-size': 20,
  'fill': '#666666',
  'text-anchor': 'middle'
});
...
hanjaArea.text(180, 220, '計 算').attr({
  'font-size': 70,
  'text-anchor': 'middle'
});
...
var m1 = hanjaArea.text(110, 305, ['셀 ', '계']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m2 = hanjaArea.text(195, 305, ['셈 ', '산']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m3 = hanjaArea.text(55, 395, ['문제를 많이 풀수록 ', '計算', '이 점점', '빨라집니다.']).attr({
  'font-size': 18,
  'fill': '#434343'
});
