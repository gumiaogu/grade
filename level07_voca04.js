...
function handlerPre() {
  location.replace('level07_voca03.html');
}
...
function handlerAft() {
  location.replace('level07_voca05.html');
}

vocaCard.use().transform('t30, 100').appendTo(topBox);

hanjaArea.text(180, 80, '4 / 43').attr({
  'font-size': 20,
  'fill': '#666666',
  'text-anchor': 'middle'
});
...
hanjaArea.text(180, 220, '規 則').attr({
  'font-size': 70,
  'text-anchor': 'middle'
});
...
var m1 = hanjaArea.text(110, 305, ['법 ', '규']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m2 = hanjaArea.text(185, 305, ['법칙 ', '칙']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m3 = hanjaArea.text(55, 395, ['정해진 ', '規則', '을 잘 따르고 지켜야', '합니다.']).attr({
  'font-size': 18,
  'fill': '#434343'
});
