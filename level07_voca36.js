...
function handlerPre() {
  location.replace('level07_voca35.html');
}
...
function handlerAft() {
  location.replace('level07_voca37.html');
}

vocaCard.use().transform('t30, 100').appendTo(topBox);

hanjaArea.text(180, 80, '36 / 43').attr({
  'font-size': 20,
  'fill': '#666666',
  'text-anchor': 'middle'
});
...
hanjaArea.text(180, 220, '重 要').attr({
  'font-size': 70,
  'text-anchor': 'middle'
});
...
var m1 = hanjaArea.text(90, 305, ['무거울 ', '중']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m2 = hanjaArea.text(195, 305, ['구할 ', '요']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m3 = hanjaArea.text(55, 390, ['형광펜으로 ', '重要', '한 부분에 밑줄을', '그었습니다.']).attr({
  'font-size': 18,
  'fill': '#434343'
});
