...
function handlerPre() {
  location.replace('level07_voca30.html');
}
...
function handlerAft() {
  location.replace('level07_voca32.html');
}

vocaCard.use().transform('t30, 100').appendTo(topBox);

hanjaArea.text(180, 80, '31 / 43').attr({
  'font-size': 20,
  'fill': '#666666',
  'text-anchor': 'middle'
});
...
hanjaArea.text(180, 220, '場 面').attr({
  'font-size': 70,
  'text-anchor': 'middle'
});
...
var m1 = hanjaArea.text(105, 305, ['마당 ', '장']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m2 = hanjaArea.text(200, 305, ['낯 ', '면']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m3 = hanjaArea.text(55, 390, ['동화를 듣고 떠오르는 ', '場面', '을 이야', '기해 봅시다.']).attr({
  'font-size': 18,
  'fill': '#434343'
});
