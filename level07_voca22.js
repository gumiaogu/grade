...
function handlerPre() {
  location.replace('level07_voca21.html');
}
...
function handlerAft() {
  location.replace('level07_voca23.html');
}

vocaCard.use().transform('t30, 100').appendTo(topBox);

hanjaArea.text(180, 80, '22 / 43').attr({
  'font-size': 20,
  'fill': '#666666',
  'text-anchor': 'middle'
});
...
hanjaArea.text(180, 220, '信 號').attr({
  'font-size': 70,
  'text-anchor': 'middle'
});
...
var m1 = hanjaArea.text(100, 305, ['믿을 ', '신']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m2 = hanjaArea.text(190, 305, ['이름 ', '호']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m3 = hanjaArea.text(55, 390, ['출발 ', '信號', '가 울리자 선수들은 달리', '기 시작했습니다.']).attr({
  'font-size': 18,
  'fill': '#434343'
});
