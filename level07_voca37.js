...
function handlerPre() {
  location.replace('level07_voca36.html');
}
...
function handlerAft() {
  location.replace('level07_voca38.html');
}

vocaCard.use().transform('t30, 100').appendTo(topBox);

hanjaArea.text(180, 80, '37 / 43').attr({
  'font-size': 20,
  'fill': '#666666',
  'text-anchor': 'middle'
});
...
hanjaArea.text(180, 220, '質 問').attr({
  'font-size': 70,
  'text-anchor': 'middle'
});
...
var m1 = hanjaArea.text(100, 305, ['바탕 ', '질']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m2 = hanjaArea.text(190, 305, ['물을 ', '문']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m3 = hanjaArea.text(55, 390, ['수업 시간에 모르는 내용이 있으면 ', '質問', '하기 전에 손을 듭니다.']).attr({
  'font-size': 18,
  'fill': '#434343'
});
