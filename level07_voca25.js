...
function handlerPre() {
  location.replace('level07_voca24.html');
}
...
function handlerAft() {
  location.replace('level07_voca26.html');
}

vocaCard.use().transform('t30, 100').appendTo(topBox);

hanjaArea.text(180, 80, '25 / 43').attr({
  'font-size': 20,
  'fill': '#666666',
  'text-anchor': 'middle'
});
...
hanjaArea.text(180, 220, '役 割').attr({
  'font-size': 70,
  'text-anchor': 'middle'
});
...
var m1 = hanjaArea.text(110, 305, ['부릴 ', '역']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m2 = hanjaArea.text(200, 305, ['벨 ', '할']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m3 = hanjaArea.text(55, 390, ['시장 놀이에서 나의 ', '役割', '은 과일', '가게 주인입니다.']).attr({
  'font-size': 18,
  'fill': '#434343'
});
