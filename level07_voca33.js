...
function handlerAft() {
  location.replace('level07_voca34.html');
}

vocaCard.use().transform('t30, 100').appendTo(topBox);

hanjaArea.text(180, 80, '33 / 43').attr({
  'font-size': 20,
  'fill': '#666666',
  'text-anchor': 'middle'
});
...
hanjaArea.text(180, 220, '正 直').attr({
  'font-size': 70,
  'text-anchor': 'middle'
});
...
var m1 = hanjaArea.text(100, 305, ['바를 ', '정']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m2 = hanjaArea.text(190, 305, ['곧을 ', '직']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m3 = hanjaArea.text(55, 390, ['正直', '한 사람은 남을 속이지 않습', '니다.']).attr({
  'font-size': 28,
  'fill': '#434343',
  'font-weight': 'bold',
});
