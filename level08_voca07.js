...
function handlerPre() {
  location.replace('level08_voca06.html');
}
...
function handlerAft() {
  location.replace('level08_voca08.html');
}

vocaCard.use().transform('t30, 100').appendTo(topBox);

hanjaArea.text(180, 80, '7 / 13').attr({
  'font-size': 20,
  'fill': '#666666',
  'text-anchor': 'middle'
});
...
a1 = hanjaArea.text(180, 220, ['先 生', ' 님']).attr({
  'font-size': 70,
  'text-anchor': 'middle'
});
a1.selectAll('tspan')[1].attr({
  'font-size': 40,
});
...
var m1 = hanjaArea.text(75, 305, ['먼저 ', '선']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m2 = hanjaArea.text(175, 305, ['날 ', '생']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m3 = hanjaArea.text(55, 395, ['先生', '님께서 숙제를 내주셨습니다.']).attr({
  'font-size': 28,
  'font-weight': 'bold',
  'fill': '#434343'
});
