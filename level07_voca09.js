...
function handlerPre() {
  location.replace('level07_voca08.html');
}
...
function handlerAft() {
  location.replace('level07_voca10.html');
}

vocaCard.use().transform('t30, 100').appendTo(topBox);

hanjaArea.text(180, 80, '9 / 43').attr({
  'font-size': 20,
  'fill': '#666666',
  'text-anchor': 'middle'
});
...
hanjaArea.text(180, 220, '方 法').attr({
  'font-size': 70,
  'text-anchor': 'middle'
});
...
var m1 = hanjaArea.text(110, 305, ['모 ', '방']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m2 = hanjaArea.text(195, 305, ['법 ', '법']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m3 = hanjaArea.text(55, 390, ['지역마다 김치를 담그는 다양한 ', '方法', '이 있습니다.']).attr({
  'font-size': 18,
  'fill': '#434343'
});
