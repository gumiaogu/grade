...
function handlerPre() {
  location.replace('level07_voca12.html');
}
...
function handlerAft() {
  location.replace('level07_voca14.html');
}

vocaCard.use().transform('t30, 100').appendTo(topBox);

hanjaArea.text(180, 80, '13 / 43').attr({
  'font-size': 20,
  'fill': '#666666',
  'text-anchor': 'middle'
});
...
hanjaArea.text(180, 220, '分 明').attr({
  'font-size': 70,
  'text-anchor': 'middle'
});
...
var m1 = hanjaArea.text(100, 305, ['나눌 ', '분']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m2 = hanjaArea.text(190, 305, ['밝을 ', '명']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m3 = hanjaArea.text(55, 390, ['그는 사람들 앞에서 자기의 의견을 ', '分明', '하게', ' 밝혔습니다.']).attr({
  'font-size': 18,
  'fill': '#434343'
});
