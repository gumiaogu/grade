...
function handlerPre() {
  location.replace('level08_voca05.html');
}
...
function handlerAft() {
  location.replace('level08_voca07.html');
}

vocaCard.use().transform('t30, 100').appendTo(topBox);

hanjaArea.text(180, 80, '6 / 13').attr({
  'font-size': 20,
  'fill': '#666666',
  'text-anchor': 'middle'
});
...
hanjaArea.text(180, 220, '生 活').attr({
  'font-size': 70,
  'text-anchor': 'middle'
});
...
var m1 = hanjaArea.text(110, 305, ['날 ', '생']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m2 = hanjaArea.text(195, 305, ['살 ', '활']).attr({
  'font-size': 20,
  'fill': '#999999'
});
...
var m3 = hanjaArea.text(55, 395, ['生活', ' 계획표를 작성할 때는 실천', '가능하도록 짜야 합니다.']).attr({
  'font-size': 28,
  'font-weight': 'bold',
  'fill': '#434343'
});
